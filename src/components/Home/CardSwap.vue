  ...
  cardRefs.value.forEach((el, i) => {
    if (el) {
      placeNow(
        ...
        makeSlot(
          i,
          props.cardDistance ?? 60,
          props.verticalDistance ?? 70,
          total,
        ),
        props.skewAmount ?? 6,
      );
...
const swap = () => {
  ...
  const [frontMaybe, ...rest] = order.value;
  const front = frontMaybe as number;
  ...
  const distX = props.cardDistance ?? 60;
  const distY = props.verticalDistance ?? 70;

  tl.addLabel(
    "promote",
    `-=${config.value.durDrop * config.value.promoteOverlap}`,
  );
  rest.forEach((idx, i) => {
    const el = cardRefs.value[idx];
    if (!el) return;
    const slot = makeSlot(i, distX, distY, cardRefs.value.length);
  ...
  tl.addLabel(
    "return",
    `promote+=${config.value.durMove * config.value.returnDelay}`,
  );
  tl.call(
    () => {
